...
public class FileUploadToTomcatController {
	 ...
	 * @param file
	 * @param folderName
	 * @return fileName (if empty returns emptyFile)
	 */
	public String uploadFileInGivenFolder(MultipartFile file, String folderName) {
		...
		if (!file.isEmpty()) {
			try {
				...
				// Creating the directory to store file
				String rootPath = System.getProperty("catalina.home");
				File dir = new File(rootPath + File.separator + "GlobalWebsiteFiles"+File.separator+""+folderName);
				if (!dir.exists())
					dir.mkdirs();
